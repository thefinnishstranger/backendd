...
blogsRouter.get('/latest', async (request, response) => {
  try {
    const mostRecentBlogs = await Blog.find().sort({ $natural: -1 }).limit(2)

    if (mostRecentBlogs.length > 0) {
      response.json(mostRecentBlogs)
    } else {
      response.status(404).json({ error: 'no blogs found' })
    }
  } catch (error) {
    console.error('error fetching the most recent blog', error);
